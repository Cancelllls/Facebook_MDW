...
(function() {
    ...
    function _load(url) {
        totalResources++;
        logStatus("Queuing: " + url.split('/').pop());
        if(resourceCache[url]) {
            ...
            var img = new Image();
            img.onload = function() {
                logStatus("Success: " + url.split('/').pop());
            ...
            img.onerror = function() {
                logStatus("FAILED: " + url.split('/').pop());
                // If an image fails to load, use a dummy transparent 1x1 pixel
                var dummy = new Image();
                dummy.src = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
                resourceCache[url] = dummy;
                loadedResources++;
                if(isReady()) {
                    readyCallbacks.forEach(function(func) { 
                      try { func(); } catch(e) { logStatus("Error in onReady (fallback): " + e.message); }
                    });
                }
            };
    ...
    function get(url) {
        return resourceCache[url] || (window._dummyImg || (window._dummyImg = new Image()));
    }
...
(function() {
    ...
    function setKey(event, status) {
        ...
        case 40: case 83: // Down or S
            key = 'DOWN'; break;
        case 88: case 75: // X or K
            key = 'JUMP'; break;
        case 90: case 74: // Z or J
            key = 'RUN'; break;
        default:
    ...
    function updateDebug(key, status) {
        var el = document.getElementById('debug-' + key.toUpperCase());
        if (el) {
            el.innerHTML = status ? '<b style="color:#fff; background:red; padding:0 4px">ON</b>' : 'OFF';
        }
    }
    ...
    window.addEventListener('blur', function() {
        for(var k in pressedKeys) updateDebug(k, false);
        pressedKeys = {};
    });
    ...
    window.input = {
        ...
        // Force key state for touch buttons
...
(function() {
	...
	var Player = Mario.Player = function(pos) {
		...
		Mario.Entity.call(this, {
			pos: pos,
			sprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/player.png', [80,32],[16,16],0),
			hitbox: [0,0,16,16]
		});
	};
  ...
  Player.prototype.setAnimation = function() {
    ...
    if (this.jumping) {
		...
		if (this.left) {
			this.sprite.img = 'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/playerl.png';
		} else {
			this.sprite.img = 'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/player.png';
		}
  };
		        ...
		        if (this.pos[0] >= this.targetPos[0]) {
		                ...
		                if (!this.exitTimer) {
		                        this.exitTimer = window.setTimeout(function() {
		                                ...
		                                player.exitTimer = null;
		                                var next = level.nextLoader || level.loader;
		                                next();
...
(function() {
  ...
  Goomba.prototype.bump = function() {
    sounds.kick.play();
    this.sprite.img = 'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/enemyr.png';
    this.flipping = true;
    this.pos[1] -= 1;
    this.vel[0] = 0;
    this.vel[1] = -2.5;
...
(function() {
  ...
  Koopa.prototype.update = function(dt, vX) {
    ...
    if (this.left) {
      this.sprite.img = 'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/enemy.png';
    } else {
      this.sprite.img = 'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/enemyr.png';
    }
...
(function() {
  ...
  var Fireball = Mario.Fireball = function(pos) {
    ...
    Mario.Entity.call(this, {
      pos: pos,
      sprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [96, 144], [8,8], 5, [0,1,2,3]),
      hitbox: [0,0,8,8]
    });
  }
  ...
  Pipe.prototype.render = function(ctx, vX, vY) {
    switch (this.direction) {
      ...
      case "LEFT":
        ...
        for (var i = 0; i < this.length-1; i++) {
          this.midsection.render(ctx, this.pos[0]+i*16, this.pos[1], vX, vY)
        }
        break;
    }
  }
...
(function() {
  var Level = Mario.Level = function(options) {
    ...
    this.nextLoader = options.nextLoader;
    ...
    this.rubbleSprite = options.rubbleSprite || function () { return new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [64,0], [8,8], 3, [0,1]); };
  ...
  Level.prototype.putQBlock = function(x, y, item) {
    ...
    rubbleSprite: function () { return new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [64,0], [8,8], 3, [0,1]) },
  ...
  Level.prototype.putCastle = function(x, y) {
    // Build a simple 5x5 castle structure using wall and brick sprites
    for(var i=0; i<5; i++) {
      for(var j=0; j<5; j++) {
        // Doorway (bottom center)
        if (i === 2 && j < 2) continue; 
        
        // Battlements (top edge, alternating)
        if (j === 4 && i % 2 !== 0) continue;
        
        // We put it in statics so Mario walks IN FRONT of the door (the door is empty) but BEHIND the castle? 
        // Wait, Mario renders AFTER statics. So Mario renders in front of statics.
        this.scenery[y-j][x+i] = new Mario.Prop([16*(x+i), 16*(y-j)], this.wallSprite);
      }
    }
  }
})();
var oneone = Mario.oneone = function() {
  ...
  level = new Mario.Level({
    playerPos: [56,192],
    loader: Mario.oneone,
    nextLoader: Mario.onetwo,
    background: "#7974FF",
    scrolling: true,
    invincibility: [144, 192, 240],
    exit: 205,
    floorSprite:  new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [0,0],[16,16],0),
    cloudSprite:  new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [0,320],[48,32],0),
    wallSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [0, 16],[16,16],0),
    brickSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [16, 0], [16,16], 0),
    brickBounceSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png',[32,0],[16,16],0),
    rubbleSprite: function () {
      return new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [64,0], [8,8], 3, [0,1])
    },
    ublockSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [48, 0], [16,16],0),
    superShroomSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [0,0], [16,16], 0),
    fireFlowerSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [0,32], [16,16], 20, [0,1,2,3]),
    starSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [0,48], [16,16], 20, [0,1,2,3]),
    pipeLEndSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [0, 128], [16,16], 0),
    pipeREndSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [16, 128], [16,16], 0),
    pipeLMidSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [0, 144], [16,16], 0),
    pipeRMidSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [16, 144], [16,16], 0),

    pipeUpMid: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [0, 144], [32,16], 0),
    pipeSideMid: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [48, 128], [16,32], 0),
    pipeLeft: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [32, 128], [16,32], 0),
    pipeTop: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [0, 128], [32,16], 0),
    qblockSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [384, 0], [16,16], 8, [0,0,0,0,1,2,1]),
    rubbleSprite: function () { return new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [64,0], [8,8], 3, [0,1]) },
    bcoinSprite: function() {
      return new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [0,112],[16,16], 20,[0,1,2,3]);
    },
    cloudSprites:[
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [0,320],[16,32],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [16,320],[16,32],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [32,320],[16,32],0)
    ],
    hillSprites: [
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [128,128],[16,16],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [144,128],[16,16],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [160,128],[16,16],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [128,144],[16,16],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [144,144],[16,16],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [160,144],[16,16],0)
    ],
    bushSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [176, 144], [48, 16], 0),
    bushSprites: [
     new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [176,144], [16,16],0),
     new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [192,144], [16,16],0),
     new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [208,144], [16,16],0)],
   goombaSprite: function() {
     return new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/enemy.png', [0, 16], [16,16], 3, [0,1]);
   },
   koopaSprite: function() {
     return new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/enemy.png', [96,0], [16,32], 2, [0,1]);
   },
   flagPoleSprites: [
     new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [256, 128], [16,16], 0),
     new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [256, 144], [16,16], 0),
     new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [128, 32], [16,16], 0)
   ]
 });
...
var onetwo = Mario.onetwo = function() {
  level = new Mario.Level({
    playerPos: [40, 176],
    loader: Mario.onetwo,
    nextLoader: Mario.onethree,
    background: "#000000",
    scrolling: true,
    exit: 210,
    floorSprite:  new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [0,32],[16,16],0),
    wallSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [32, 32],[16,16],0),
    brickSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [16, 0], [16,16], 0),
    brickBounceSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png',[32,0],[16,16],0),
    ublockSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [48, 0], [16,16],0),
    qblockSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [384, 0], [16,16], 8, [0,0,0,0,1,2,1]),
  ...
  // Floor layout
  level.putFloor(0, 147);
  level.putFloor(151, 230); // Pit at 147-150

  // Standard Underground Ceiling (Gap for Warp Zone at the end)
  for(var i=0; i<180; i++) {
    level.putWall(i, 2, 2);
  }

  // Intro: Bricks overhead
  for(var i=10; i<25; i++) {
  ...
  level.putPipe(45, 13, 3);
  level.putPipe(55, 13, 4);

  // Floating Brick Platforms
  for(var i=65; i<75; i++) {
    level.putBrick(i, 8, null);
    if(i == 70) level.putQBlock(i, 8, new Mario.Mushroom([i*16, 8*16]));
  }
  level.putGoomba(70, 12);

  // Large brick staircase structure
  for(var i=85; i<95; i++) {
    level.putWall(i, 13, i-84); // Steps up
  }
  for(var i=95; i<105; i++) {
    level.putWall(i, 13, 105-i); // Steps down
  }

  // More enemies and bricks
  level.putGoomba(120, 12);
  level.putGoomba(125, 12);
  for(var i=130; i<145; i++) {
    level.putBrick(i, 7, null);
  }
  
  // The Jump to Flagpole area
  level.putWall(160, 13, 4);
  level.putWall(165, 13, 6);
  level.putWall(170, 13, 8);

  // Flagpole area
  level.putFlagpole(205);
  level.putCastle(209, 12);
};
...
var onethree = Mario.onethree = function() {
  ...
  vX = 0;
  level.putFloor(0, 50);
  level.putFloor(60, 50);
  level.putFloor(120, 60);
  
  // Athletic level trees
  for(var i=0; i<10; i++) {
    level.putWall(20 + i*15, 10, 3);
  }

  level.putFlagpole(170);
  level.putCastle(175, 12);
};
...
var onefour = Mario.onefour = function() {
  ...
  player.pos[1] = level.playerPos[1];
  vX = 0;
  level.putFloor(0, 100);
  for(var i=0; i<100; i++) {
    level.putWall(i, 2, 2); // Ceiling
    if (i % 10 === 0) level.putWall(i, 13, 2); // Some "lava" pits
  }
  
  level.putFlagpole(90);
  level.putCastle(95, 12);
};
...
var oneonetunnel = Mario.oneonetunnel = function() {
  level = new Mario.Level({
    ...
    scrolling: false,
    coinSprite: function() {
      return new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png', [0,96],[16,16], 6,[0,0,0,0,1,2,1]);
    },
    floorSprite:  new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [0,32],[16,16],0),
    wallSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [32, 32],[16,16],0),
    brickSprite: new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [16, 0], [16,16], 0),
    ...
    LPipeSprites:[
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [32,128],[16,16],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [32,144],[16,16],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [48,128],[16,16],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [48,144],[16,16],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [64,128],[16,16],0),
      new Mario.Sprite('https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png', [64,144],[16,16],0),
    ]
...
//load our images
resources.load([
  'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/player.png',
  'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/enemy.png',
  'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/tiles.png',
  'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/playerl.png',
  'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/items.png',
  'https://raw.githubusercontent.com/reruns/mario/gh-pages/sprites/enemyr.png',
]);
...
function init() {
  var getSrc = function(url) {
    return (window.MARIO_ASSETS && window.MARIO_ASSETS[url]) ? window.MARIO_ASSETS[url] : url;
  };
  music = {
    overworld: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/aboveground_bgm.ogg')),
    underground: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/underground_bgm.ogg')),
    clear: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/stage_clear.wav')),
    death: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/mariodie.wav'))
  };
  sounds = {
    smallJump: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/jump-small.wav')),
    bigJump: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/jump-super.wav')),
    breakBlock: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/breakblock.wav')),
    bump: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/bump.wav')),
    coin: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/coin.wav')),
    fireball: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/fireball.wav')),
    flagpole: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/flagpole.wav')),
    kick: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/kick.wav')),
    pipe: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/pipe.wav')),
    itemAppear: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/itemAppear.wav')),
    powerup: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/powerup.wav')),
    stomp: new Audio(getSrc('https://raw.githubusercontent.com/reruns/mario/gh-pages/sounds/stomp.wav'))
  };
  ...
  var startGame = function(levelName) {
